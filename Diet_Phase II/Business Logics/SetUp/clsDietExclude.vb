Public Class clsDietExclude
    ...
    Public Sub LoadForm()
        Try
            If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_Z_OEXD) = False Then
                oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Exit Sub
            End If
            oForm = oApplication.Utilities.LoadForm(xml_Z_OEXD, frm_Z_OEXD)
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_Z_OEXD Then
                    ...
                    Case True
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CLICK
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                oDBDataSource = oForm.DataSources.DBDataSources.Item("@Z_OEXD")
                                If pVal.ItemUID = "3" Then
                                    intSelectedMatrixrow = pVal.Row
                                    If (oDBDataSource.GetValue("U_Code", 0).ToString() = "") Then
                                        BubbleEvent = False
                                        oApplication.SBO_Application.SetStatusBarMessage("Enter Exclude Code to Proceed...", SAPbouiCOM.BoMessageTime.bmt_Medium, True)
...
#Region "Data Events"
    Public Sub FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean)
        Try
            ...
            If oForm.TypeEx = frm_Z_OEXD Then
                    ...
                    Case False
                        Select Case BusinessObjectInfo.EventType
                            Case SAPbouiCOM.BoEventTypes.et_FORM_DATA_LOAD
                                ...
                                oMatrix.Columns.Item("V_1").Width = 300
                        End Select
                End Select
            End If
        Catch ex As Exception
...
#Region "Function"
    ...
    Private Sub initialize(ByVal oForm As SAPbouiCOM.Form)
        Try
            oDBDataSource = oForm.DataSources.DBDataSources.Item("@Z_OEXD")
            oDBDataSourceLines = oForm.DataSources.DBDataSources.Item("@Z_EXD1")
            ...
            '    oCombobox.ValidValues.Add("0", "--Select--")
            '    Dim strCurrYear As Integer = System.DateTime.Now.Year
            '    For intRow As Integer = strCurrYear To strCurrYear + 1
            ...
            oRecordSet.DoQuery("Select IsNull(MAX(DocEntry),0) +1 From [@Z_OEXD]")
        ...
        Try
            Select Case aForm.PaneLevel
                Case "0"
                    ...
                    oDBDataSourceLines = aForm.DataSources.DBDataSources.Item("@Z_EXD1")
            End Select
            oMatrix.FlushToDataSource()
...
#Region "Validations"
    Private Function validation(ByVal aForm As SAPbouiCOM.Form) As Boolean
        Try
            ...
            '    oApplication.Utilities.Message("Select Year to Exclude...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            '    Return False
            'End If
            ...
            '    If oDBDataSourceLines.GetValue("U_ExDate", index) <> "" Then
            '        blnRowExists = True
            '    End If
            'Next
            'If Not blnRowExists Then
            '    oApplication.Utilities.Message("Add Exclude Date in Matrix to Proceed...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            '    Return False
            'End If


            oRecordSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            strQuery = "Select 1 As 'Return',DocEntry From [@Z_OEXD]"
            strQuery += " Where "
            strQuery += " U_Code = '" + oDBDataSource.GetValue("U_Code", 0).Trim() + "' And DocEntry <> '" + oDBDataSource.GetValue("DocEntry", 0).ToString() + "'"
            oRecordSet.DoQuery(strQuery)

            If Not oRecordSet.EoF Then
                oApplication.Utilities.Message("Diet Exclude Already Exist for Selected Code...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
            ...
            '                oApplication.Utilities.Message("Diet Exclude Date Exist for Selected Year...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            '                Return False
            '            End If
            '        End If
            '    Next
...
#Region "Disable Controls"
    ...
    Private Sub EnableControls(ByVal oForm As SAPbouiCOM.Form, ByVal blnEnable As Boolean)
        Try
            oForm.Items.Item("10").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
            oForm.Items.Item("16").Enabled = blnEnable
