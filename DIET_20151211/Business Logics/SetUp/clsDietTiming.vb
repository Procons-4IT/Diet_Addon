...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_Z_OTTI Then
                                ...
                                If pVal.ItemUID = "3" And pVal.ColUID <> "U_Code" Then
                                    If pVal.Row = oMatrix.VisualRowCount Then
                                        ...
                                        If strCode.Trim().Length = 0 Then
                                            oApplication.Utilities.Message("Enter CheckUp Code to Proceed...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                            BubbleEvent = False
                                            Exit Sub
                                        End If
                                    End If
                    ...
                    If pVal.BeforeAction = True Then
                        ...
                        Else
                            If intMatrixSelectedRow > 0 Then
                                If oApplication.Utilities.ValidateRemoveSetup(oApplication.Utilities.getMatrixValues(oMatrix, "U_Code", intMatrixSelectedRow), "Timing") = False Then
                                    oApplication.Utilities.Message("Calories Adjustment : " & oApplication.Utilities.getMatrixValues(oMatrix, "U_Code", intMatrixSelectedRow) & " already mapped to Transaction / Customer.", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                    BubbleEvent = False
                                    Exit Sub
                                End If
                            End If
...
#Region "Right Click Event"
    ...
    Public Sub RightClickEvent(ByRef eventInfo As SAPbouiCOM.ContextMenuInfo, ByRef BubbleEvent As Boolean)
        ...
        If oForm.TypeEx = frm_Z_OTTI Then
            ...
            For index As Integer = 1 To oMatrix.VisualRowCount - 1
                ...
                If index = oMatrix.VisualRowCount Then
                    ...
                    If strCode.Trim().Length = 0 And (strName.Trim.Length > 0) Then
                        _retVal = False
                        oApplication.Utilities.Message("Check Session Code Cannot be Empty...in Row No :" + index.ToString(), SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                        Exit For
                    End If
    ...
    Public Function validatedata() As Boolean
        Try
            ...
            For index As Integer = 1 To oMatrix.VisualRowCount
                Dim strCode As String = oApplication.Utilities.getMatrixValues(oMatrix, "U_Code", index)
                Dim intMin As Integer = Convert.ToInt32(IIf(oApplication.Utilities.getMatrixValues(oMatrix, "U_MinTime", index) = "", 0, oApplication.Utilities.getMatrixValues(oMatrix, "U_MinTime", index)))
                Dim intMax As Integer = Convert.ToInt32(IIf(oApplication.Utilities.getMatrixValues(oMatrix, "U_MaxTime", index) = "", 0, oApplication.Utilities.getMatrixValues(oMatrix, "U_MaxTime", index)))
                For intRow As Integer = 1 To oMatrix.VisualRowCount
                    If index <> intRow Then
                        ...
                        If strCode = strCode1 Then
                            _retVal = False
                            oApplication.Utilities.Message("Check Session Code Already defined...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                            Return _retVal
                        End If
                    End If
                Next
                If intMax < intMin Then
                    _retVal = False
                    oApplication.Utilities.Message("Max Time Should be greater than or Equal to Min Time...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return _retVal
                End If
            Next
            Return _retVal
